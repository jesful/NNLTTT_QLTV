				...
				if (JOptionPane.showOptionDialog(contentPane, "Bạn Có Chắc Muốn Thêm Thông Tin Mượn Sách", "Mượn Sách", JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE, null, options, options[1]) == JOptionPane.YES_OPTION) 
				{
					...
					{		
						...
						try 
						{
							...
							if(rs.getInt(1) == 0)
							{
								if(MuonTraModel.InsertMuon(IDTextField.getText(), IDDGTextField.getText(), NgayM))
								{
									if(MuonTraModel.InsertCTMuon(IDTextField.getText(), IDSachTextField.getText(), 0))
									{
										if(MuonTraModel.UpdateSLS(IDSachTextField.getText()))
										{
											JOptionPane.showMessageDialog(contentPane, "Mượn Sách Thành Công", "Mượn Sách", JOptionPane.INFORMATION_MESSAGE);
										}
										else
										{
											JOptionPane.showMessageDialog(contentPane, "Mượn Sách Thất Bại", "Mượn Sách", JOptionPane.ERROR_MESSAGE);
										}
									}
									else
									{
								...
								{
									JOptionPane.showMessageDialog(contentPane, "Thêm Mượn Sách Thất Bại", "Thêm Mượn Sách", JOptionPane.ERROR_MESSAGE);
								}
							}
							else
							{
